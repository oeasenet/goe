...
func registerRoutes(httpKernel contract.HTTPKernel, cacheManager contract.CacheManager, logger contract.Logger) {
	// Get the Fiber app
	app := httpKernel.App()
	
	logger.Info("Registering cache HTTP demo routes")
	
	// Product routes
	api := app.Group("/api")
	api.Get("/products/:id", getProductHandler(cacheManager))
	api.Post("/products", createProductHandler(cacheManager))
	api.Delete("/cache", clearCacheHandler(cacheManager))
	api.Get("/stats", getCacheStatsHandler(cacheManager))
...
func getProductHandler(cacheManager contract.CacheManager) fiber.Handler {
...
func createProductHandler(cacheManager contract.CacheManager) fiber.Handler {
	return func(c fiber.Ctx) error {
		logger := http.GetLogger(c)
		store := cacheManager.Store()
...
func clearCacheHandler(cacheManager contract.CacheManager) fiber.Handler {
	return func(c fiber.Ctx) error {
		logger := http.GetLogger(c)
		store := cacheManager.Store()
...
func getCacheStatsHandler(cacheManager contract.CacheManager) fiber.Handler {
	return func(c fiber.Ctx) error {
		store := cacheManager.Store()
